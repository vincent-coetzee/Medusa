...
class BufferBrowserViewController: NSViewController
    ...
    public var annotatedBytes: AnnotatedBytes?
        ...
        didSet
            {
            self.bufferBrowserView.annotatedBytes = self.annotatedBytes
            self.byteCountLabel.stringValue = String(format: "%d bytes",self.annotatedBytes!.sizeInBytes)
            }
        }
...
//                {
//                self.buffer?.fill(atByteOffset: offset,with: Byte(self.allocationCount),count: sizeInBytes)
